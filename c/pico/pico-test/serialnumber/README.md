...
But you don't really need it! To get Pico serial number one-time, it is enough to run a simple [hello world](../hello/source.c) in VS code > **Run** in bottom right corner: you will see the serial number displayed in VS Code Terminal when it flashes it onto Pico.
...
You can alternatively connect to Pico (to see the log) using screen emulator (`screen` or `minicom` on Mac).
...
After changing the USB connection, I was able to run the code on Pico. 
